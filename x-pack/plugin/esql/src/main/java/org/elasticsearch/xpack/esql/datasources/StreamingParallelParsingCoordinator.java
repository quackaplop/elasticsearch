    ...
    static final class StreamingParallelIterator implements CloseableIterator<Page> {
        ...
        private void runParserOnce() {
            ...
            try {
                ...
                // - lastSplit: every chunk is aligned to a record boundary by the segmentator.
                // - recordAligned: chunks always start on a record boundary.
                // - readSchema: planner-resolved typed column layout (PR #148756); null falls back
                // to per-file inference.
                FormatReadContext ctx = FormatReadContext.builder()
                    .projectedColumns(projectedColumns)
                    .batchSize(batchSize)
                    .errorPolicy(errorPolicy)
